...
do
	...
	echo $line | \
		...
		sed "s/ )/)/g" | \
                sed "s/ ()//g" | \
                sed "s/()//g" | \
		sed "s/{\"name\":\"cpu_temperature\",\"instance\":\"\\/sys\\/devices\\/platform\\/coretemp.0\\/hwmon\\/hwmon[0-9]\\/temp1_input\",\"markup\":\"none\",\"full_text\":\"can't read temp\",\"separator\":false,\"separator_block_width\":0},//g" | \
